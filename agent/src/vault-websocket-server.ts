...
import { VaultTradingAgent, VaultTradeResult, TokenInfo } from './vault-agent.js';
...
export class VaultWebSocketServer {
  private app: express.Application;
  private server: any;
  private io: SocketIOServer;
  private vaultAgent: VaultTradingAgent;
  ...
  constructor() {
    ...
    this.vaultAgent = new VaultTradingAgent();
    this.logger = new Logger('VaultWebSocketServer');
  ...
  async start(port: number = 3001): Promise<void> {
    try {
      ...
      this.server.listen(port, () => {
        this.logger.info(`🚀 Vault WebSocket Server running on port ${port}`);
  ...
  private async shutdown(): Promise<void> {
    this.logger.info('Shutting down Vault WebSocket Server...');
...
const isMainModule = import.meta.url === `file://${process.argv[1]}` || 
                     ...
                     process.argv[1].includes('vault-websocket-server');
