...
import { VaultLangGraphAgent } from './vault-langgraph-agent.js';
...
export class VaultLangGraphWebSocketServer {
  private app: express.Application;
  private server: any;
  private io: SocketIOServer;
  private vaultAgent: VaultLangGraphAgent;
  ...
  constructor() {
    ...
    this.vaultAgent = new VaultLangGraphAgent();
    this.logger = new Logger('VaultLangGraphWebSocketServer');
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/api/vault/status', async (req, res) => {
      try {
        ...
        // Get vault status using the agent
    ...
    this.io.on('connection', (socket: any) => {
      ...
      socket.emit('status', {
        ...
        data: {
          ...
          vaultAddress: '0xB6C58FDB4BBffeD7B7224634AB932518a29e4C4b',
          chainId: '11155111',
          serverType: 'vault-langgraph-server',
          features: {
            langgraph: true,
            mcp: true,
            tools: true,
      ...
      socket.on('trade_request', async (data: { message: string; clientId?: string }) => {
        try {
          ...
          socket.emit('status', {
            type: 'processing',
            data: { message: 'Processing trade request with LangGraph...' },
            timestamp: new Date()
          });

          // Process the trade request using LangGraph
          ...
          if (result.success && result.data?.operation && ['buy', 'sell'].includes(result.data.operation)) {
      ...
      socket.on('wallet_connect', async (data: { walletAddress?: string }) => {
        try {
  ...
  async start(port: number = 3001): Promise<void> {
    try {
      ...
      this.server.listen(port, () => {
        this.logger.info(`🚀 Vault LangGraph WebSocket Server running on port ${port}`);
        ...
        this.logger.info(`🤖 Agent Type: LangGraph + LangChain + MCP`);
        this.logger.info(`🛠️ Features: Tools, Wallet Connection, Real-time Updates`);
      });
  ...
  private async shutdown(): Promise<void> {
    this.logger.info('Shutting down Vault LangGraph WebSocket Server...');
...
const isMainModule = import.meta.url === `file://${process.argv[1]}` || 
                     ...
                     process.argv[1].includes('vault-langgraph-server');

if (isMainModule) {
  const server = new VaultLangGraphWebSocketServer();
  const port = parseInt(process.env.PORT || '3001');
  
  console.log('🚀 Starting Vault LangGraph WebSocket Server...');
